...
Module HLayer (l : Layer) <: Layer.
  ...
  Definition opT := horizOpT nat l.opT.
  Definition State := horizState nat l.State.
  Definition step := horizStep nat l.step.

  Definition indexValid (i : nat) := i = 0 \/ i = 1.
...
Module LockedCounterHAPI := HLayer LockedCounterAPI.
Module CounterHAPI := HLayer CounterAPI.
...
Module AbsCounterH <: LayerImpl LockedCounterHAPI CounterHAPI.

  Definition absR := horizAbsR nat AbsCounter.absR.

  Definition compile_ts (ts : @threads_state CounterHAPI.opT) := ts.

  Theorem compile_ts_no_atomics :
    forall (ts : @threads_state CounterHAPI.opT),
      no_atomics_ts ts ->
      no_atomics_ts (compile_ts ts).
  Proof.
    unfold compile_ts; eauto.
  ...
  Theorem absR_ok :
    op_abs absR LockedCounterHAPI.step CounterHAPI.step.
  Proof.
    eapply horizAbsR_ok.
    apply AbsCounter.absR_ok.
  Qed.
  ...
  Theorem compile_traces_match :
    forall ts,
      no_atomics_ts ts ->
      traces_match_abs absR LockedCounterHAPI.initP LockedCounterHAPI.step CounterHAPI.step (compile_ts ts) ts.
  Proof.
    unfold compile_ts, traces_match_abs; intros.
    eexists; intuition idtac.
    eapply trace_incl_abs; eauto.
  ...
  Theorem absInitP :
    forall s1 s2,
      LockedCounterHAPI.initP s1 ->
      absR s1 s2 ->
      CounterHAPI.initP s2.
  Proof.
    eapply horizAbsR_initP_ok.
    eapply AbsCounter.absInitP.
  Qed.
